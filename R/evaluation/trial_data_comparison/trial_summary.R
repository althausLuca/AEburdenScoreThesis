...
trial_folder <- "data/trials_2000/"
method_results_folder <- "data/model_results_2000/"
...
for(file in files){
  ...
  k_value <- as.numeric(sub(".*_k_([0-9.]+).*", "\\1", file))
  s_value <- as.numeric(sub(".*_s_([0-9]+(?:\\.[0-9]+)?).*", "\\1", file))
  ...
  hist( rowSums(df.control==0),breaks=20 , main = paste("control", " k=",k_value," s=",s_value))
  hist( rowSums(df.treatment==0),breaks=20 , main = paste("treatment", " k=",k_value," s=",s_value))
  ...
  p_values <- p_values[,!(names(p_values) %in% c("quantile_regression.nid","wilcoxon" ))]
