...
distributions_plot <- function(distributions, x, Y ,  distributions_title = "Distributions", ecdf_title = "ECDF" , max_n_trials = Inf , text_size_factor = 10) {
  ...
  p <- ggplot(combined_data, aes(x = x, y = value, linewidth  = line_type, color = line_type, group = interaction(trial, line_type))) +
    geom_line(data = subset(combined_data, line_type == "Distributions" & x > 0), alpha = 0.01 ,linewidth =0.08 ) +
    geom_line(data = subset(combined_data, line_type == "Distributions" & x <= 0), alpha = 0.01 ,linewidth =0.08 ) +
