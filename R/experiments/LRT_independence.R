...
for (i in seq_along(trials_data_loaders)) {
  ...
  plot(B_T_P_wilcoxon$B, B_T_P_wilcoxon$T, xlab = "B", ylab = "T", main = "",
       pch = 1, col = "blue"
    , asp = 1, xlim = c(min(B_T_P_wilcoxon$B) - 0.2, max(B_T_P_wilcoxon$B) + 0.2), ylim = c(min(B_T_P_wilcoxon$T) - 0.2, max(B_T_P_wilcoxon$T) + 0.2)
  ...
  print(corr_t <- cor.test(abs(B_T_P_t$B), B_T_P_t$T, method = "pearson"))
