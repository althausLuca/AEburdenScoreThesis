...
for(file in files){
  ...
  l_value <- as.numeric(sub(".*_l_([0-9]+(?:\\.[0-9]+)?).*", "\\1", file))
  ...
  hist( rowSums(df.control==0),breaks=20 , main = paste("control", " k=",k_value," l=",l_value))
  hist( rowSums(df.treatment==0),breaks=20 , main = paste("treatment", " k=",k_value," l=",l_value))
