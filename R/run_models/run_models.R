...
run_models <- function(trial_file_path, models = config$DEFAULT_MODELS, recompute = config$CLEAR_ALL) {
  ...
  result_path <- gsub("data/trials/", "results/", trial_file_path)
  result_path <- gsub("data/", "results/", result_path) # for the case of a special trial folder
  ...
  errors <- add_models(model_computer, models)
  print(errors)
  if (length(errors) > 0) {
